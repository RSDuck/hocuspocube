...
of cpReadPointer, 0x34, 4:
    ...
    write: fifoReadPointer.adr = swapHalves(val)
of cpWritePointer, 0x38, 4:
    read: swapHalves(uint32 fifoWritePointer)
    write: fifoWritePointer.adr = swapHalves(val)

proc cpNotifyFifoBurst*(data: openArray[uint32]): bool =
    if cr.gpLink:
        assert (data.len mod 8) == 0

        cmdlistParser.queueData(toOpenArray(cast[ptr UncheckedArray[byte]](unsafeAddr data), 0, data.len * 4 - 1))
